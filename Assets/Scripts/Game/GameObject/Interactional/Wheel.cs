        ...
        public void Interact(IInteractionalObject obj)
        {
            ...
            else
            {
                if((_collider.transform.position.x != _defaultPos.position.x))
                    _collider.transform.position = Vector3.MoveTowards(_collider.transform.position, new Vector3(_defaultPos.position.x, _collider.transform.position.y, _collider.transform.position.z), Time.deltaTime * 2);
            }
